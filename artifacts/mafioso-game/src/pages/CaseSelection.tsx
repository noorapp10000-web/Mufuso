...
export default function CaseSelection() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-b from-[#130212] to-[#20051C]">
      <div className="sticky top-0 z-10 bg-[#130212]/95 backdrop-blur-sm px-5 pt-12 pb-4 border-b border-gold/10">
        <div className="flex items-center gap-3 mb-4">
          <button onClick={goHome} className="p-2 rounded-lg bg-[#350A22] border border-gold/20 active:scale-95 transition-transform">
  ...
  return (
    <button
      ...
      className={`w-full text-right rounded-xl border transition-all duration-200 active:scale-[0.98] overflow-hidden
        ${c.isUnlocked
          ? "bg-[#350A22] border-gold/25 hover:border-gold/60 hover:bg-[#430B24]"
          : "bg-[#1C0418]/60 border-gold/10 opacity-55"
        }`}
    >
      <div className="flex items-stretch">
        <div className="w-20 flex-shrink-0 bg-[#1C0418] flex items-center justify-center p-2">
          <c.SvgScene />
        </div>
        <div className="flex flex-col justify-between flex-1 p-3">
          ...
          {c.isUnlocked && (
            <div className="flex gap-2 mt-2">
              <span className="text-[10px] font-cairo text-gold/40 bg-[#1C0418] border border-gold/10 rounded px-2 py-0.5">
